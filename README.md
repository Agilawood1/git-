...
- 注
  ...
  - 当**自己**开发新分支 a 时，主分支 main 上有新改动，当前分支需要用到时，用 **rebase**
  - 当**主分支**合并其他开发分支功能时，用 **merge**。
  - **公共分支**上**一定不要**用 rebase，因为这会改写历史，导致其他队友的代码与远程仓库完全脱节，造成严重的同步混乱。
  - 当你对自己分支进行 rebase 操作后，需要立即 force push 一次（当且仅当这一次），然后继续正常开发、正常 pull、正常 push，就不会再需要 force push 了
